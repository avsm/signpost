...
/*        if ((ret = avahi_entry_group_add_service_subtype(group, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, 0, name, "_printer._tcp", NULL, "_magic._sub._printer._tcp") < 0)) {
        ...
        ret = avahi_entry_group_add_record(group, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, 
                ...
                "haris.local", AVAHI_DNS_CLASS_IN, AVAHI_DNS_TYPE_A, AVAHI_DEFAULT_TTL, 
                "\x0a\x0a\x0a\x0a", 4); 
                 if (ret >= 0) 
                     printf("Published DNS-SD hostname alias [haris.local]\n");
                 else {
                      fprintf(stderr, "Failed to add CNAME record [haris.local]: %s\n", avahi_strerror(ret));
                      goto fail;
                 }
